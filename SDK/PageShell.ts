...
export class PageShell
{
    ...
    loadjQueryVersion()
    {
        if (!Misc.isNullOrEmpty(PageShell.JQUERY_VERSION)) return;
        new VirtualFunction({
            fnName: 'getjQVersion',
            fnContent: `
                if(PageShell.JQUERY_VERSION == '')
                {
                    try 
                    {
                        ...
                        // 2. Tenta pelo '$' como fallback
                        // 3. Usa o operador ?. para evitar erro se fn não existir
                        const version = window.jQuery?.fn?.jquery || window.$?.fn?.jquery;

                        if (version) 
                        {
                            PageShell.JQUERY_VERSION = version;
                        } 
                        else 
                        {
                            PageShell.JQUERY_VERSION = 'Não encontrado';
                            console.warn('jQuery carregado, mas propriedade de versão não encontrada.');
                        }
                    } 
                    catch (e) 
                    {
    ...
    public expandSplitView()
    {
        ...
        {
            var list: string[] = []
            for (var i = 0; i < self.appContainer.classList.length; i++)
                list.push(self.appContainer.classList[i])

            self.appContainer.classList.remove(...list);
            self.appContainer.classList.add('col-6');

            self.splitContainer.classList.remove(...list);
