...
export class VirtualFunction
{
     ...
     */
    call(...argValues: any[]): VirtualFunction
    {
        var argNamesStr = this.argNamesStr();
        

        const argNames = argNamesStr.split(','); // array real: ['a', 'b', etc]

        const fn = new Function(...argNames, this.functionBodyContent);
    ...
    private argValuesStr(...argValues: string[]): string
    {
        ...
        for (var a = 0; a < argValues.length; a++)
            argValuesStr += `${JSON.stringify(argValues[a])}, `;
