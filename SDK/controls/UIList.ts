...
export class UIListBinder extends WidgetBinder
{
    ...
    getWidgetValue()
    {
        ...
        if(this.isTargetDefined())
            return item.value[this.valueProperty]
...
export class UIList extends Widget implements IBindable
{
    ...
    {
        return `
<${this.containerTag} id="ui-list" class="list-group ${this.containerDivClass}">
</${this.containerTag}>`
    }
     ...
     */
    constructor({ name, multiSelect, containerClass, containerTag }:
        {
            name: string,
            multiSelect?: boolean,
        ...
        })
    {
        ...
        this.containerTag = Misc.isNullOrEmpty(containerTag) ? 'div' : containerTag
    }
    ...
    public fromList(viewModels: Array<any>, valueProperty?: string, displayProperty?: string): void
    {
        ...
        if (viewModels == null || viewModels == undefined || viewModels.length == 0) 
        {
            try
            {
                const templateProvider = this.itemTemplateProvider();
                if (templateProvider != null)
                {
                    const customItem = templateProvider.getListItemTemplate(this, null);
        ...
        for (var i = 0; i < viewModels.length; i++)
        {
            ...
            {
                const defaultItemTemplate = new ListItem(
                    `${i + 1}`,
                    text,
                    value);
            ...
            {
                const templateProvider = this.itemTemplateProvider();
                const customItem = templateProvider.getListItemTemplate(this, viewModel);
                this.addItem(customItem);
            }
        }
    }
    ...
    {
        this.divContainer = this.elementById('ui-list');
    }
    ...
    public addItem(item: IListItemTemplate): UIList
    {
        ...
        var view: HTMLElement = item.itemTemplate();
