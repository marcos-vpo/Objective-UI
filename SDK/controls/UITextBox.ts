...
export class UITextBox extends Widget implements IBindable
{
    ...
    {
        var contentGroup = `
        <span id="entrySymbol" style="height: 34px; background: lightgray" class="input-group-text" > </span>
        <input id="entryInput" ${this.readOnly? 'readonly' : ''} ${this.disabled? 'disabled' : ''}  ${this.toUpperCase ? 'oninput="this.value = this.value.toUpperCase();"' : ''} class="form-control form-control-sm"  placeholder="Entry placeholder">`
        ...
        <span  style="height: 34px; background: lightgray" class="input-group-text"> 
...
${contentGroup}
    ...
    onWidgetDidLoad(): void
    {
        ...
        else
        {
            ...
            if (!Misc.isNullOrEmpty(this.txInput.style.height))
                this.spanSymbol.style.height = this.txInput.style.height
        }
