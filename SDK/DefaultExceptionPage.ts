...
{
    private static redirectDivId: string = ''
    public static enableRedirect(targetDivId: string)
    {
        DefaultExceptionPage.redirectDivId = targetDivId
    }

    public static disableRedirect()
    {
        DefaultExceptionPage.redirectDivId = ''
    }
    ...
    constructor(error: Error)
    {
        ...
        for (var i = 0; i < errorsStr.length; i++)
        {
            ...
            if (msg.indexOf('at') == 0)
            {
                ...
                msg = msg.replace(codePath,
                    `<span class="badge text-primary"> ${codePath} </span>`);
            }

            msg = msg.replace(origin, '').replace('(', '').replace(')', '').trim();
            rawHtml += `${msg} <br/>`;
        }
        ...
        if (Misc.isNullOrEmpty(DefaultExceptionPage.redirectDivId))
        {
            document.body.prepend(c);
        }
        else
        {
            document.getElementById(DefaultExceptionPage.redirectDivId).appendChild(c);
        }
    }
}